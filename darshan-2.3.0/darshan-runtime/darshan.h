...
#ifdef HISTORY
extern struct darshan_history_header *darshan_hheader;
#define HIST_SIZE ((1024*1024 - sizeof(struct histent*) - sizeof(uint32_t))/(sizeof(struct darshan_history_data)))
struct histent {
    struct histent *hist_next;
    uint32_t hist_offset;
    struct darshan_history_data hist_ent[HIST_SIZE];
};
struct history {
    double hist_start;
    double hist_last;
    uint64_t hist_total;
    struct histent *hist_head;
    struct histent *hist_tail;
};
#endif /* HISTORY */
