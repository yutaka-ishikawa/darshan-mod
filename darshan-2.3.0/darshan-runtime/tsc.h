 ...
 * HISTORY:
 *      -  fopen, fclose, and fgets are replaced with darshan_XXX
...
static inline uint64_t tick_helz_auto(void)
{
    ...
    fp = darshan_fopen("/proc/cpuinfo", "r");
    if (fp == 0) {
	helz = 0;
    }
    else {
	...
	while (darshan_fgets(buf, sizeof (buf), fp) != 0) {
	    int rc = sscanf(buf, fmt1, &ls);
	    if (rc == 1) { break; }
	}
	helz = (uint64_t)(ls * 1.0e6);
	darshan_fclose(fp); fp = 0;
    }
...
static uint64_t tick_helz(double *p_helz)
{
