...
#ifdef HISTORY
/* The below entry is dynamically created */
#define HISTMAGIC_SIZE		8
#define HISTMAGIC_STRING	"HIST"
struct darshan_history_header {
    char	magic[HISTMAGIC_SIZE]; /* "HIST" 64 bit boundary */
    uint32_t 	nfiles;
    uint32_t	rank;
};
...
struct darshan_history_file {
    ...
    double	hfile_rstart;
    double	hfile_wstart;
    uint32_t	hfile_nopen; /* number of times */
