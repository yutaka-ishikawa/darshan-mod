...
#ifdef HISTORY
static int getfile_internal_204h(darshan_fd fd, struct darshan_job *job, 
    struct darshan_file *file);
#endif /* HISTORY */
...
#ifdef HISTORY
struct darshan_history darshan_hist;
static struct darshan_history_util *darshan_hutil;
static struct darshan_history_header darshan_hheader;
static struct darshan_history_file *darshan_hfile;
static struct darshan_history_data *darshan_hdata;
static int getfile_internal_204h(darshan_fd fd, struct darshan_job *job, 
    struct darshan_file *file)
{
    int		i, iret, ret;
    ...
    darshan_hist.hutil = 0;
    if (darshan_hutil) { free(darshan_hutil); darshan_hutil = 0; }
    if (darshan_hfile) { free(darshan_hfile); darshan_hfile = 0; }
    if (darshan_hdata) { free(darshan_hdata); darshan_hdata = 0; }
...
#ifdef HISTORY_DEBUG
	printf("getfile_internal_204h: ret(%d) returns\n", ret);
#endif /* HISTORY_DEBUG */
	return 0;
    }
    if (strncmp(darshan_hheader.magic, HISTMAGIC_STRING, HISTMAGIC_SIZE) != 0) {
	/* it might be still darshan file */
	darshan_log_seek(fd, curpos);
	return iret;
    }

    /* darshan_history */
    ret = darshan_log_read(fd, &tmp32, sizeof(uint32_t));
    if (ret < 0) {
	return ret;
    }
    darshan_hheader.nfiles = ntohl(tmp32);
#ifdef HISTORY_DEBUG
    printf("nfiles=%d\n", darshan_hheader.nfiles);
#endif /* HISTORY_DEBUG */
    ret = darshan_log_read(fd, &tmp32, sizeof(uint32_t));
    if (ret < 0) return ret;
    darshan_hheader.rank = ntohl(tmp32);
#ifdef HISTORY_DEBUG
    printf("rank=%d\n", darshan_hheader.rank);
#endif /* HISTORY_DEBUG */

    /* darshan_history_file */
    dhfsz = SIZE_HISTORY_FILE * darshan_hheader.nfiles;
    darshan_hfile = (struct darshan_history_file *) malloc(dhfsz);
    dhusz = SIZE_HISTORY_UTIL * darshan_hheader.nfiles;
    darshan_hutil = (struct darshan_history_util *) malloc(dhusz);
    if (darshan_hfile == NULL || darshan_hutil == NULL) {
	printf("Cannot reserve memory (%ld + %ld) byte\n", dhfsz, dhusz);
	return -1;
    }
    ret = darshan_log_read(fd, darshan_hfile, dhfsz);
    if (ret < 0) {
	return -1;
    }
    memset(darshan_hutil, 0, dhusz);
    nhistdata = 0;
    ...
    darshan_hdata = (struct darshan_history_data*) malloc(histsz);
    if (darshan_hdata == NULL) {
	printf("Cannot reserve memory\n");
	return -1;
    }
    ret = darshan_log_read(fd, darshan_hdata, histsz);
    /* linking to data */
    nhistdata = 0;
    for (i = 0; i < darshan_hheader.nfiles; i++) {
	if ((szr = darshan_hutil[i].hutil_read)) {
	    darshan_hutil[i].hutil_rdata = &darshan_hdata[nhistdata];
	    nhistdata += szr;
	}
	if ((szw = darshan_hutil[i].hutil_write)) {
	    darshan_hutil[i].hutil_wdata = &darshan_hdata[nhistdata];
	    nhistdata += szw;
	}
    }
    darshan_hist.nfiles = darshan_hheader.nfiles;
    darshan_hist.hutil = darshan_hutil;
    return iret;
}
#endif /* HISTORY */
