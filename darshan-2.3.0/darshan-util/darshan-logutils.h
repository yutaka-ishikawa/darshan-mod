...
struct darshan_history_util {
    ...
    uint32_t	hutil_nfopen;
    uint32_t	hutil_read; /* each has two entries, i.e., read and write */
    uint32_t	hutil_write;	 
    struct darshan_history_data	*hutil_rdata;
    struct darshan_history_data	*hutil_wdata;
};
struct darshan_history {
    uint32_t	nfiles;
    struct darshan_history_util	*hutil;
};
extern struct darshan_history darshan_hist;
#define SIZE_HISTORY_UTIL sizeof(struct darshan_history_util)
#endif /* HISTORY */
#endif
