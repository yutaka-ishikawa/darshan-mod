...
#ifdef HISTORY
#define OPTION_HISTORY  (1 << 7)  /* history extended by RIKEN AICS  */
#define OPTION_ALL (\
  OPTION_BASE|\
  OPTION_TOTAL|\
  OPTION_PERF|\
...
#ifdef HISTORY
    fprintf(stderr, "    --history : history (extended by RIKEN AICS)\n");
#endif /* HISTORY */
...
#ifdef HISTORY
    if (mask & OPTION_HISTORY) {
	while ((ret = darshan_log_getfile(file, &job, &cp_file)) == 1) {
	    int		i, j;
	    if (darshan_hist.nfiles == 0) continue;
#ifdef HISTORY_DEBUG
	    printf("darshan_hist.nfiles = %d\n", darshan_hist.nfiles);
#endif /* HISTORY_DEBUG */
...
#ifdef HISTORY
	if (!(mask & OPTION_HISTORY)) continue;
	if (darshan_hist.nfiles) {
	    int		i, j;
#ifdef HISTORY_DEBUG
	    printf("darshan_hist.nfiles = %d\n", darshan_hist.nfiles);
#endif /* HISTORY_DEBUG */
